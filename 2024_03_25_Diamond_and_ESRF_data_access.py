...
def plot_multi_file_one_pos_time(source, scatter_type, position, n_files, n_frames, t_offset, t_step, ):
    ...
    if source == 'DLS' and scatter_type == 'SAXS':
        ...
        fig,ax = plt.subplots()
        c = ax.imshow(np.log(filter_data), vmin = 2, vmax = 4, aspect = 'auto')
        ...
        t_vals = t_vals[position::n_frames] # take only the time correspoinding to a certain position
...
t_vals, filter_data, q_values = plot_multi_file_one_pos_time('DLS', 'SAXS', position = 11, n_files = 1, n_frames = 14, t_offset = 1, t_step = 1)

#plot_one_file_all_pos(ESRF_SAXS_path, 'ESRF', step = 5)

#plot_one_file_one_pos(ESRF_SAXS_path, 'ESRF', 5)

#t_vals, t = plot_multi_file_one_pos_time(DLS_SAXS_path, 'DLS', 1, 1, 1)

data, q_values, t, start_time= read_nxs(ESRF_drop_dir / ESRF_SAXS_file_list[0], 'ESRF', timing)
